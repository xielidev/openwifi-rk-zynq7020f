...
module system_top (
  ...
  /* === [已注释] 不使用的 HDMI、I2S 以及 SPDIF 物理引脚接口 ===
  output                  hdmi_out_clk,
  output                  hdmi_vsync,
  output                  hdmi_hsync,
  output                  hdmi_data_e,
  ...
  /* === [已注释] iic_mux 物理管脚（从物理端口列表中删除，避免物理管脚冲突报错！） ===
  ...
  /* === [已注释] 删除无物理引脚连接的 I2C 缓冲模拟器，直接在下方虚拟拉高它 ===
  ...
  // 虚拟拉高内部 I2C 选通线输入端，防止底层模块报错并避免物理引脚悬空冲突
  assign iic_mux_scl_i_s = 2'b11;
  assign iic_mux_sda_i_s = 2'b11;
  ...
  system_wrapper i_system_wrapper (
    ...
    // === [已注释] 底层系统包装中不再使用的 HDMI/音频 连接信号线 ===
    // .hdmi_data (hdmi_data),
